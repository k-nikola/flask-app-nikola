...
class CosmonautForm(FlaskForm):
    ...
    departure_date = DateField(label="When would you like to depart?",validators=[DataRequired()], default=date.today)
    return_date = DateField(label="When would you like to return?",validators=[DataRequired()])
    ...
    def validate_age(self, attempted_age):
        if attempted_age.data<18 or attempted_age.data>150:
            raise ValidationError("You cannot book an interstellar vacation with that age. Only those older than 18 and younger than 150 years can book a vacation like this.")
    def validate_departure_date(self, attempted_departure):
        if attempted_departure.data>self.return_date.data or attempted_departure.data<date.today():
            raise ValidationError("Travelling through time yet isn't possible. Pick a departure date that is not in the past and that is before the return date.")
        booked_slots = InterstellarTraveller.query.filter_by(departure_date=attempted_departure.data, destination=self.destination.data).count()
        if booked_slots>=5:
            raise ValidationError(f"There are no available slots on {self.destination.data} for this departure date, please pick another date to start your interstellar journey!")